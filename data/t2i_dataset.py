...
class T2IIterableDataset(DistributedIterableDataset):
    def __init__(
        ...
        local_rank=0, world_size=1, num_workers=8, data_status=None, vae_transform=None,
    ):
        """
        data_dir_list: list of data directories contains parquet files
        num_used_data: list of number of sampled data paths for each data directory
        vae_transform: optional VAE-specific transform for image generation tasks
        """
        super().__init__(dataset_name, local_rank, world_size, num_workers)
        self.transform = transform
        self.vae_transform = vae_transform if vae_transform is not None else transform
    ...
    def get_data_paths(self, data_dir_list, num_used_data):
        return get_parquet_data_paths(data_dir_list, num_used_data, rank=self.local_rank, world_size=self.world_size)
                ...
                with fs.open_input_file(parquet_file_path) as f:
                    ...
                    for row_group_id in row_group_ids_:
                        ...
                        for row_idx, row in df.iterrows():
                            ...
                            caps_token = [self.tokenizer.encode(v, add_special_tokens=False) for _, v in caption_dict.items()]
                            if len(caps_token) == 0:
                                print(f'no caption in rg#{row_group_id}, {parquet_file_path}')
                                caption_token = self.tokenizer.encode(' ', add_special_tokens=False)
                            else:
