...
from .transforms import SigLIPImageTransform, VAEImageTransform
...
class DataConfig:
    def __init__(
        ...
        vae_image_downsample=16,  # downsample=16, latent_patch_size=1, so it is 16*1=16
        max_latent_size=64,
        vit_patch_size=14,
...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            if 'image_transform_args' in dataset_args.keys():
                transform = SigLIPImageTransform(**dataset_args.pop('image_transform_args'))
                dataset_args['transform'] = transform
            if 'vit_image_transform_args' in dataset_args.keys():
                vit_transform = SigLIPImageTransform(**dataset_args.pop('vit_image_transform_args'))
                dataset_args['vit_transform'] = vit_transform
            if 'vae_image_transform_args' in dataset_args.keys():
                vae_transform = VAEImageTransform(**dataset_args.pop('vae_image_transform_args'))
                dataset_args['vae_transform'] = vae_transform
            else:
                # If VAE transform is not specified, use default transform
                dataset_args['vae_transform'] = transform
    ...
    def to_tensor(self, sequence_status):
        ...
        if not self.use_flex:
            ...
            pad_len = self.max_tokens_per_batch - sequence_len
        ...
        if len(sequence_status['vae_image_tensors']) > 0:
            ...
            # Add original images for REPA (convert to tensor format)
            from torchvision import transforms
            original_transform = transforms.Compose([
                transforms.Resize((256, 256)),
                transforms.ToTensor()
            ])
            # Get original images from the sample if available
            if hasattr(self, 'current_sample') and 'raw_image_list' in self.current_sample:
                raw_images = self.current_sample['raw_image_list']
                original_image_tensors = [original_transform(img) for img in raw_images]
                original_image_sizes = [item.shape for item in original_image_tensors]
                original_max_image_size = [max(item) for item in list(zip(*original_image_sizes))]
                original_images = torch.zeros(size=(len(original_image_tensors), *original_max_image_size))
                for i, original_image_tensor in enumerate(original_image_tensors):
                    original_images[i, :, :original_image_tensor.shape[1], :original_image_tensor.shape[2]] = original_image_tensor
                data['original_images'] = original_images
            else:
                # Fallback: use padded_images as original_images
                data['original_images'] = padded_images.clone()
        ...
        if len(sequence_status['packed_vit_images']) > 0:
            # Save image list directly without concatenation
            data['packed_vit_images'] = sequence_status['packed_vit_images']  # Image list
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if sequence_status['curr'] + num_tokens > self.max_tokens_per_batch:
            ...
            # Check if batch generation condition is met
            should_yield = False
            if self.micro_batch_size is not None:
                # Control by sample count
                if len(batch_data_indexes) >= self.micro_batch_size:
                    should_yield = True
            else:
                # Control by token count (original logic)
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        for item in sequence_plan:
            ...
            if item['type'] == 'text':
                ...
                # Save original image directly without patchify
                # Calculate patch count for sequence length calculation
                h, w = image_tensor.size(1), image_tensor.size(2)
                num_patches_h = h // self.data_config.vit_patch_size
                num_patches_w = w // self.data_config.vit_patch_size
...
class SimpleCustomBatch:
    def __init__(self, batch):
        ...
        if "original_images" in data.keys():
    ...
    def pin_memory(self):
        ...
        if hasattr(self, 'original_images'):
            self.original_images = self.original_images.pin_memory()
        ...
        if hasattr(self, 'packed_vit_images'):
            # packed_vit_images is now an image list, need to process individually
            if isinstance(self.packed_vit_images, list):
                self.packed_vit_images = [img.pin_memory() for img in self.packed_vit_images]
            else:
    ...
    def cuda(self, device):
        ...
        if hasattr(self, 'original_images'):
            self.original_images = self.original_images.to(device)
        ...
        if hasattr(self, 'packed_vit_images'):
            # packed_vit_images is now an image list, need to process individually
            if isinstance(self.packed_vit_images, list):
                self.packed_vit_images = [img.to(device) for img in self.packed_vit_images]
            else:
                self.packed_vit_images = self.packed_vit_images.to(device)
    ...
    def to_dict(self):
        ...
        if hasattr(self, 'original_images'):
            data['original_images'] = self.original_images
