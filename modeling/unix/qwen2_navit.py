...
from .repa_utils import REPAProjector, REPAEncoderManager, REPALoss
...
class Qwen2Config(_Qwen2Config):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.repa_enc_type = repa_enc_type
...
class PackedAttention(Qwen2Attention):
    ...
    def forward_train(
        ...
        if isinstance(attention_mask, List):
            ...
            # fallback: dense causal mask
            seq_len = packed_query_states.size(1)
            causal_mask = torch.tril(torch.ones(seq_len, seq_len, dtype=torch.bool, device=packed_query_states.device))
            packed_attn_output = scaled_dot_product_attention(
                ...
                attn_mask=causal_mask.unsqueeze(0),  # (1, seq_len, seq_len)
            )
...
class Qwen2Model(Qwen2PreTrainedModel):
    def __init__(self, config):
        ...
        # REPA components - lazy initialization (only for training)
        self.use_repa = getattr(config, 'use_repa', False)
        self.repa_encoder_depth = getattr(config, 'repa_encoder_depth', 8)
        self.repa_config = config  # Save config for lazy init
        self.repa_encoder_manager = None
        self.repa_loss_fn = None

        # Initialize weights and apply final processing
        self.post_init()

    def _init_repa(self, config):
        """Initialize REPA components"""
        # Get REPA configuration with defaults
        repa_enc_type = getattr(config, 'repa_enc_type', 'dinov2-vit-b')
        repa_projector_dim = getattr(config, 'repa_projector_dim', 2048)
        
        # Create encoder manager with projectors
        self.repa_encoder_manager = REPAEncoderManager(
            enc_type=repa_enc_type,
        ...
        z_dims = self.repa_encoder_manager.z_dims
        
        # Create loss function using projectors from encoder manager
        self.repa_loss_fn = REPALoss(
            encoders=self.repa_encoder_manager.encoders,
            encoder_types=self.repa_encoder_manager.encoder_types,
            z_dims=z_dims,
            projectors=self.repa_encoder_manager.projectors
        )
        
        print(f"REPA initialized: enc_type={repa_enc_type}, "
                f"projector_dim={repa_projector_dim}, "
                f"encoder_depth={self.repa_encoder_depth}")
    ...
    def forward_train(
        ...
        # Store intermediate features for REPA
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            # Extract gen features at specified layer for REPA
            if return_intermediate_features and layer_idx == intermediate_depth and 'packed_gen_token_indexes' in extra_inputs:
                gen_token_indexes = extra_inputs['packed_gen_token_indexes']
                gen_features = packed_sequence[gen_token_indexes]  # Extract only gen tokens
                
                # Reshape gen features to (N, T, D) for REPA
                # We need to determine the batch size and sequence length
                batch_size = len(sample_lens)
                gen_tokens_per_sample = gen_features.shape[0] // batch_size
                hidden_size = gen_features.shape[1]
                intermediate_features = gen_features.reshape(batch_size, gen_tokens_per_sample, hidden_size)
        ...
        # Compute REPA loss if enabled (lazy init on first use)
        repa_loss = None
        if self.use_repa and original_images is not None:
            if self.repa_loss_fn is None:
                self._init_repa(self.repa_config)
            if self.repa_loss_fn is not None:
                # Extract encoder features from original images using real DINOv2
                with torch.no_grad():
                    encoder_features = self.repa_encoder_manager.extract_features(original_images)

                # Compute REPA loss using intermediate features
                if intermediate_features is not None:
                    repa_loss = self.repa_loss_fn(intermediate_features, encoder_features)
                else:
                    # If no intermediate features, use final output
                    repa_loss = self.repa_loss_fn(final_output.unsqueeze(1), encoder_features)
        
        # Return REPA loss if available
        if repa_loss is not None:
            return final_output, repa_loss
        else:
            return final_output
